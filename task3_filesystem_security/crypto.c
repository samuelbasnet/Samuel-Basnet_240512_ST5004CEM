 ...
 * crypto.c
 * ---------
 * Implementation of SHA-256 (public-domain style, compact single-file
 * implementation) plus the XOR stream cipher and salt generation used
 ...
 * Standard FIPS 180-4 algorithm. This is a well-known, widely taught
 * compact implementation pattern (32-bit words, 64 rounds, 8 hash
 * state words) reproduced here so the project has no external crypto
 * library dependency and can be compiled with plain gcc.
 * ========================================================= */
...
static void sha256_final(SHA256_CTX *ctx, unsigned char hash[]) {
    ...
    for (i = 0; i < 4; i++) {
        for (int j = 0; j < 8; j++) {
            hash[i + (j * 4)] = (unsigned char)((ctx->state[j] >> (24 - i * 8)) & 0x000000ff);
        }
    }
}
...
/* =========================================================
 * Salt generation, password hashing, XOR cipher
 * ========================================================= */

void generate_salt(unsigned char salt[SALT_LEN]) {
    /* Seeded once per process in main(); rand() is adequate for a
     * coursework demo but NOT for production secrets (see security
     * analysis for the recommendation to use a CSPRNG such as
     * /dev/urandom or arc4random in a real system). */
...
void hash_password(const char *password, const unsigned char salt[SALT_LEN],
    ...
    unsigned char buffer[SALT_LEN + MAX_PASSWORD];
    size_t pass_len = strlen(password);
    memcpy(buffer, salt, SALT_LEN);
    memcpy(buffer + SALT_LEN, password, pass_len);
    sha256(buffer, SALT_LEN + pass_len, out_hash);
}

void xor_cipher(unsigned char *buffer, size_t len, const char *passphrase) {
    unsigned char key[HASH_LEN];
    sha256((const unsigned char *)passphrase, strlen(passphrase), key);

    for (size_t i = 0; i < len; i++) {
        buffer[i] ^= key[i % HASH_LEN];
    }
}
