 ...
 *
 * A simple interactive TCP client for the key-value store server.
 * See docs/protocol_doc.md for the full protocol spec.
 *
 * COMPILE (Linux / WSL / Ubuntu VM):
 *   gcc -Wall -o client client.c
 * RUN:
 *   ./client [server_ip] [port]     (defaults: 127.0.0.1, 5050)
 */
 ...
 * Returns 1 on success, 0 on EOF (stdin closed / Ctrl+D), so the caller
 * can exit cleanly instead of looping forever on empty reads. */
